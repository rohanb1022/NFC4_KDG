...
import { useAuthStore } from "../store/useAuthStore"; // <-- Imported logout
...
export default function InstituteDashboard() {
  ...
  const { fetchStudents, students } = useInstitutionStore();
  const { logout } = useAuthStore(); // <-- Using logout from store
  ...
  const handleShareClick = async (walletId) => {
    try {
      const response = await axios.get(
        `http://localhost:3000/api/v1/student/details/${walletId}`
      );
      setSelectedStudent(response.data);
      setShowCertificateForm(true);
    } catch (error) {
          ...
          <div className="p-6 mt-6 border border-gray-300 rounded-md bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-4">
              ...
              <input
                ...
                value={certificateData.grade}
                onChange={(e) =>
                  setCertificateData({ ...certificateData, grade: e.target.value })
                }
                className="px-4 py-2 border rounded-md"
              />
              <input
                ...
                value={certificateData.issuedOn}
                onChange={(e) =>
                  setCertificateData({ ...certificateData, issuedOn: e.target.value })
                }
                className="px-4 py-2 border rounded-md"
              />
              <div className="flex gap-2 mt-2">
                <Button className="bg-blue-600 text-white" onClick={handleIssueCertificate}>
                  Issue
                </Button>
